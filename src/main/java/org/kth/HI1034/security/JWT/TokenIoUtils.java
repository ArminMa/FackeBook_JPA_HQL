...
public class TokenIoUtils implements Serializable {
	...
	public static final Long TOKEN_DURATION_SECONDS = (60L * 60L * 24L * 7L); // 1 week
	public static final Long TOKEN_CREATION_BUFFER_SECONDS = (60L * 5L); // 5 min
...
//	private static String tokenHeader;
	...
	public static String createJwtAuth(
			String issuer,
			String audience,
			String subject,
			List<String> roles,
	...
	public static String createJwt(
		...
		return generateToken(generateClaims( issuer, audience, subject, roles, jsonPayload));
	}
...
//			Jwts.parser().setSigningKey(key).parseClaimsJws(token.trim());
	...
	public static String getSubject(String jwsToken) {
		if (isValid(jwsToken)) {
			Jws<Claims> claimsJws = Jwts.parser().parseClaimsJws(jwsToken);
			return claimsJws.getBody().getSubject();
		}
		return null;
	}
	...
	@SuppressWarnings("unchecked")
	public static List<String> getRoles(String jwsToken) {
		if (isValid(jwsToken)) {
			Jws<Claims> claimsJws = Jwts.parser().parseClaimsJws(jwsToken);
			return (List<String>) claimsJws.getBody().get(CLAIM_KEY_ROLES);
		}
