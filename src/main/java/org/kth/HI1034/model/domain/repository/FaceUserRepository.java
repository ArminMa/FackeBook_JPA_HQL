...
public interface FaceUserRepository extends JpaRepository<FaceUser, Long>, JpaSpecificationExecutor<FaceUser> {
	...
	@Query(value = "select U FROM FaceUser U WHERE U.username = :theUserName")
	FaceUser findByUsername(@Param("theUserName") String userName);
...
//			(SELECT spouseAuthor FROM Author spouseAuthor WHERE spouseAuthor = auth.spouse)
	...
	@Query(value = "select UF FROM FaceUser UF " +
			"where UF.username = :username or UF.email = :eMail" )
	List<FaceUser> findUserByUsernameOrEmail(
			@Param("username") String UserName,
			@Param("eMail") String email);

	@Query(value = "select distinct UF FROM FaceUser UF " +
			"where UF.username = :username or UF.email = :eMail" )
	FaceUser findOneUserByUsernameOrEmail(
			@Param("username") String UserName,
			@Param("eMail") String email);
