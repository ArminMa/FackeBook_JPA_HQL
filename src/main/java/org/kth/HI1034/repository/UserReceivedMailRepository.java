...
public interface UserReceivedMailRepository extends JpaRepository<UserReceivedMail, Long>, JpaSpecificationExecutor<Long> {
	...
	List<UserReceivedMail> findAllReceivedMailByUserId(@Param("userId") Long userID);
	...
	@Modifying
	@Transactional
	@Query(value = "delete from UserReceivedMail urm where urm.pk.receivingUser.id = :userId" )
	void deleteThisRowByUserId(@Param("userId") Long userID);
